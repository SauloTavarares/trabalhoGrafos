...
public class Digrafo {
    ...
    public Digrafo(int V) {
        if (V < 0) {
            throw new IllegalArgumentException("Número de vértices no dígrafo deve ser não negativo");
        }
        this.V = V;
        this.A = 0;
        adj = new ArrayList[V];
    ...
    /**
     * Inicializa um dígrafo à partir de um arquivo de dados. O formato é o
     * número de vértices V e o número de arestas A seguido por pares de pares
     * de vértices.
     *
    ...
    public Digrafo(In in) {
        ...
        //"for" que adiciona as arestas
        for (int i = 0; i < A; i++) {
            int v1 = in.readInt();
            int v2 = in.readInt();
            if (v1 < 0 || v1 >= V) {
                throw new IndexOutOfBoundsException("vértice " + v1 + " não está entre 0 e " + (V - 1));
            }
            if (v2 < 0 || v2 >= V) {
                throw new IndexOutOfBoundsException("vértice " + v2 + " não está entre 0 e " + (V - 1));
            }
